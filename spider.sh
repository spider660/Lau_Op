...
# --- Permanent hap.pem handling (safe even if domain not pointed yet) ---
sudo mkdir -p /etc/haproxy
sudo touch /etc/haproxy/hap.pem
sudo chmod 600 /etc/haproxy/hap.pem
sudo chown root:root /etc/haproxy/hap.pem

sudo bash -c '
CRT="/etc/xray/xray.crt"
KEY="/etc/xray/xray.key"
OUT="/etc/haproxy/hap.pem"

if [ -s "$CRT" ] && [ -s "$KEY" ] \
  && head -n 1 "$CRT" | grep -q "BEGIN CERTIFICATE" \
  && head -n 1 "$KEY" | grep -Eq "BEGIN (EC |RSA )?PRIVATE KEY"; then
  cat "$CRT" "$KEY" > "$OUT"
  chmod 600 "$OUT"
  chown root:root "$OUT"
  systemctl restart haproxy >/dev/null 2>&1 || true
else
  echo "INFO: SSL cert/key not ready yet; hap.pem will be built after domain points."
fi
'
# --- end hap.pem handling ---
