#!/usr/bin/env bash
# ======================================================================
# Spider Store Universal Installer
# Compatible with all Ubuntu (18.04+) and Debian (9+)
# ======================================================================
set -euo pipefail
...
# Ensure iptables-legacy where nftables is default (Ubuntu 22+, Debian 11+)
...
# Ensure netfilter-persistent available
apt-get update -y
apt-get install -y iptables-persistent netfilter-persistent

# Fix OpenVPN service naming
...
[Unit]
Description=OpenVPN compatibility service
After=network.target
...
if ! apt-cache policy haproxy | grep -q "Installed: (none)"; then
...
sudo add-apt-repository "$LATEST_PPA" -y
fi
sudo apt update
...
echo "deb [signed-by=/usr/share/keyrings/haproxy.debian.net.gpg] http://haproxy.debian.net $(lsb_release -cs)-backports main" | sudo tee /etc/apt/sources.list.d/haproxy.list
sudo apt-get update
sudo apt-get install haproxy -y
else
...
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://wa.me/+254112011036"}],[{"text":"ᴜᴘᴅᴀᴛᴇs","url":"https://t.me/spid_3r"}],[{"text":"sᴜᴘᴘᴏʀᴛ","url":"https://t.me/spid_3r"}]]}&parse_mode=html'
